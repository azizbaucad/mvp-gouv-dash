...
export default function DcirePage(props) {
  const gstyle = gird.style;
  const { dcire } = direction;
  ...
  return (
    <DashboardLayout activeMenu={'account-dcire'}>
      <HighlightModal
        onOpen={onOpen}
        onClose={onClose}
        isOpen={isOpen}
          ...
          {statusList.map((status, i) => (
            ...
            >
              ...
              <Stack mt={2}>
                ...
                {highlights?.filter(
                  (highl) => highl.status.name === status.name
                )?.length === 0 && (
